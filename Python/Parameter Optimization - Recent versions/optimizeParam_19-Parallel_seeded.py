from lib import RDKit_Wrapper_12_sMMFF as rdw
...
for c in range(len(cutoffs)):
    cutoff = cutoffs[c]
    folder = f'Parameter Optimization - v19.{c} - HighRes'     
    ...
    # # -----------------------------------------------------------------------------
    # for loss_type_ind in range(2):
    #     print("------------------------------------------------------")
    #     print(f"Cutoff = {cutoff}")
    #     print("Extracting and re-organizing results ...")

    #     loss_types = ['$E_{MMFF}$', '$SASA$', 'Reference']
    #     loss_type = loss_types[loss_type_ind]

    #     # Calculate loss for each molecule
    #     if loss_type == '$E_{MMFF}$':
    #         loss_ind = 4
    #         loss_type_name = 'Energy'
    #     elif loss_type == '$SASA$':
    #         loss_ind = 5
    #         loss_type_name = 'SASA'
    ...
    #         'val':[results_pk[i][loss_ind] for i in range(len(results_pk))],
    #         })
    ...
    #     # normalize = lambda arr: (arr - np.nanmin(arr))/(np.nanmax(arr) - np.nanmin(arr)) if (np.nanmax(arr) - np.nanmin(arr)) != 0 else arr
    #     # normalize = lambda arr, zero_min=True: (arr - np.nanmin(arr))/(np.nanmax(arr) - np.nanmin(arr)) if zero_min else arr/(np.nanmax(arr) - np.nanmin(arr))
    ...
    #         mol_loss_lin = results['val'][mol_results].to_numpy()
    #         mol_loss = np.reshape(mol_loss_lin, (len(HBC_HBC_range), len(HBC_R_range), n_runs))
    #         mol_loss = np.sum(mol_loss, axis=2)
    #         # mol_loss = np.tril(mol_loss)
    #         # mol_loss[mol_loss==0] = np.nan
    #         mol_loss_dict.update({labels[i]:mol_loss})
            
    #         mol_loss_norm = normalize(mol_loss)
    #         mol_loss_norm_dict.update({labels[i]:mol_loss_norm})

    #     #---------------------------------------------------------------------
    #     # Plotting total loss (un-normalized and normalized)
    #     print("Extracting and plotting total loss ...")

    #     loss = pd.read_csv(f'{folder}/Loss ({loss_type_name}) as a function of HBC repulsion parameters - {n_runs} runs.csv', index_col=0).to_numpy()
    #     # loss.to_numpy()

    #     # # Replacing zeros with nans for plot
    #     # loss = np.tril(loss)
    #     # loss[loss==0] = np.nan

    #     parameter_max = 500

    #     if loss_type == '$E_{MMFF}$' or loss_type == '$SASA$':
    #         max_param_ind = np.where(HBC_HBC_range > parameter_max)[0][0]
    #         # ext = lambda array: np.nanmax(array[0:max_param_ind, 0:max_param_ind])
    #         def ext(array):
    #             # find standard deviation and mean of array
    #             std = np.nanstd(array)
    #             mu = np.nanmean(array)
    #             # replace any values further than 2 standard deviations from the mean with mu+2*std
    #             array[array > mu + 2*std] = mu + 2*std
    #             array[array < mu - 2*std] = mu - 2*std
    #             return np.nanmax(array[0:max_param_ind, 0:max_param_ind])
    #         extrema_type = "Max"
    #     else:
    #         max_param_ind = np.where(HBC_HBC_range > parameter_max)[0][0]
    #         ext = lambda array: np.nanmin(array[0:max_param_ind, 0:max_param_ind])
    #         extrema_type = "Min"

    #     extrema = np.where(loss == ext(loss))
    #     n_ext = len(extrema[0]) # number of extrema
    ...
    #     plt.xlabel('HBD-HBA Relative Force', fontweight='bold', fontsize=16)
    #     plt.ylabel('HBD-R Relative Force', fontweight='bold', fontsize=16)
    #     plt.title(f'Total {loss_type} - Repulsion Cutoff = {cutoffs[c]} Ang', fontweight='bold', fontsize=14)
    #     plt.legend()
    #     plt.savefig(f'{folder}/Loss ({loss_type}) as a function of HBC repulsion parameters - {n_runs} runs.png', dpi = 300, bbox_inches='tight')
    #     # plt.show()

    #     loss_df = pd.DataFrame(loss, index = HBC_HBC_range, columns = HBC_R_range)
    #     loss_df.to_csv(f'{folder}/Loss as a function of HBC repulsion parameters - {n_runs} runs.csv')
            
    #     # Finding normalized total loss
    #     sig_labels = []     # molecules with significant ranges in energy (not just noise)
    #     for m in range(num_mol):
    #         mol_name = labels[m]
    #         mol_loss = mol_loss_dict[mol_name]
    #         loss_range = np.nanmax(mol_loss) - np.nanmin(mol_loss)
    #         if loss_range/np.abs(np.nanmin(mol_loss)) >=  [0.05, 0.01][loss_type_ind]:
    #             sig_labels.append(mol_name)
    #     sig_labels = np.array(sig_labels)
    #     num_sig_mol = len(sig_labels)

    #     norm_loss_all = np.array([mol_loss_norm_dict[sig_labels[i]] for i in range(num_sig_mol)])
    #     norm_loss = np.sum(norm_loss_all, axis=0)

    #     # finding global extreme using sum of normalized losses
    #     extrema = np.where(norm_loss == ext(norm_loss))
    #     n_ext = len(extrema[0]) # number of extrema
    ...
    #         )
    #     for m in range(num_mol):
    #         mol_loss = mol_loss_dict[labels[m]]
    ...
    #                 color = ('red' if significant else 'white'),
    ...
    #     plt.xlabel('HBD-HBA Relative Force', fontweight='bold', fontsize=16)
    #     plt.ylabel('HBD-R Relative Force', fontweight='bold', fontsize=16)
    #     plt.title(f'Normalized Total {loss_type} - ${extrema_type}(loss) = {round(ext(norm_loss), 2)}$\n Repulsion Cutoff = {cutoffs[c]} Ang', fontweight='bold', fontsize=14)
    #     plt.legend(fontsize=12)
    #     plt.savefig(f'{folder}/Normalized loss ({loss_type}) as a function of HBC repulsion parameters - {n_runs} runs.png', dpi = 300, bbox_inches='tight')
    #     # plt.show()
    ...
    #     #---------------------------------------------------------------------
    #     # See loss contours for each molecule
    #     print("Plotting un-normalized molecule loss ...")
    ...
    #         ax = axes[m//num_col, m%num_col]
    #         cf = ax.contourf(
    #             HBC_HBC_range,
    #             HBC_R_range,
    #             np.transpose(mol_loss), 
    #             # levels = levels,
    ...
    #         ax.set_title(f'{labels[m]}\n(${extrema_type}$ = {int(ext(mol_loss))}, x = {int(mol_loss[extrema[0][0], extrema[1][0]])})')
    ...
    #     fig.suptitle(f'Molecule {loss_type} ({n_runs} runs) - Repulsion Cutoff = {cutoffs[c]} Ang', fontweight='bold', fontsize=14)
    #     plt.tight_layout()
    #     plt.savefig(f'{folder}/Molecule loss ({loss_type}) as a function of HBC repulsion parameters (un-normalized) - {n_runs} runs.png',bbox_inches='tight')
    #     # plt.show()
    ...
    #     fig.suptitle(f'Normalized Molecule {loss_type} ({n_runs} runs) - Repulsion Cutoff = {cutoffs[c]} Ang', fontweight='bold', fontsize=14)
    #     plt.tight_layout()
    #     plt.savefig(f'{folder}/Molecule loss ({loss_type}) as a function of HBC repulsion parameters (normalized) - {n_runs} runs.png',bbox_inches='tight')
    #     # plt.show()

    #     # -----------------------------------------------------------------------------
    #     # Saving optimal parameters per molecule
    #     optim_param = np.zeros((num_mol+1, 2))
    #     for i in range(num_mol):
    #         mol_loss = mol_loss_dict[labels[i]]
            
    #         mol_extrema = np.where(mol_loss == ext(mol_loss))
            
    #         # optim_param[i,:] = [HBC_HBC_range[mol_extrema[0][1]], HBC_R_range[mol_extrema[1][1]]] 
    #         a = HBC_HBC_range[mol_extrema[0][0]]; b = HBC_R_range[mol_extrema[1][0]]
    #         optim_param[i,:] = [a,b] 
            
    #     optim_param[-1,:] = [HBC_HBC_range[extrema[0][0]], HBC_R_range[extrema[1][0]]]  # adding global minimum

    #     optim_param_df = pd.DataFrame(optim_param, index = list(labels)+['Global'], columns = ['HBC_HBC', 'HBC_R'])
    #     optim_param_df.to_csv(f'{folder}/Optimized ({loss_type}) HBC repulsion parameters.csv')
        
    # # -----------------------------------------------------------------------------
    # # Testing different functional forms for the combined loss function
    # # -----------------------------------------------------------------------------
    # for loss_type_ind in range(2):
    #     print("------------------------------------------------------")
    #     print(f"Cutoff = {cutoff}")
    #     print(f"Combined loss function {loss_type_ind+1} ...")
    #     print("Extracting and re-organizing results ...")

    #     loss_types = ['$E_{MMFF}$', '$SASA$', 'Combined-', 'Combined+']
    ...
    #         # mol_loss1 = np.tril(mol_loss1)
    #         # mol_loss1[mol_loss1==0] = np.nan
    ...
    #         # mol_loss2 = np.tril(mol_loss2)
    #         # mol_loss2[mol_loss2==0] = np.nan   
    #         mol_loss2_dict.update({labels[i]:mol_loss2})
            
    #         if loss_type_ind == 0:
    #             mol_loss_norm = normalize(mol_loss2) - normalize(mol_loss1) # (SASA - Energy) with the objective to maximize - range is now [-1,1]
    #         else:
    ...
    #     sig_labels = []     # molecules with significant ranges in energy OR SASA (not just noise)
    #     sig_type = []       # list for type of singificant loss 
    #     for m in range(num_mol):
    #         mol_name = labels[m]
    ...
    #     # -----------------------------------------------------------------------------
    #     # Saving optimal parameters per molecule
    #     optim_param = np.zeros((num_mol+1, 3))
    #     for i in range(num_mol):
    ...
    #     optim_param[-1,:] = [HBC_HBC_range[extrema[0][0]], HBC_R_range[extrema[1][0]], np.nan]  # adding global minimum

    #     optim_param_df = pd.DataFrame(optim_param, index = list(labels)+['Global'], columns = ['HBC_HBC', 'HBC_R', 'Significance'])
    #     optim_param_df.to_csv(f'{folder}/Optimized ({loss_type}) HBC repulsion parameters.csv')
    ...
    for loss_type_ind in range(1):
        ...
        for m in range(num_mol):
            ...
            for a in range(n_atoms):
                atm_type = rdw.isHB(molecule, a)
                if atm_type == 'HBD':
                    num_HBD += 1
                elif atm_type == 'HBA':
                    num_HBA += 1
            n_HBC = num_HBD + num_HBA
            if num_HBD >=1 and n_HBC > 2: # 1 HBA will be the electronegative atom bonded to H, the other will be an available H-bonding site
                HB_mol[m] = 1
