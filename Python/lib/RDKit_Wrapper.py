...
def generateConformer(smilesString,repulsion_params=None,xyzPath=None,calc_energy=False):
    ...
    repulsion_params : list of floats, optional
        List the repulsion parameters. The list must have the following format:
            [HBD_HBA_f,HBC_R_f,cutoff]  
        where:
            HBD_HBA_f : float
                Force constant for HBD-HBA repulsion.
            HBC_R_f : float
                Force constant for HBD/HBA-R repulsion.
            cutoff : float
                Cutoff distance for the repulsion.
