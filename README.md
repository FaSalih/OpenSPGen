...
![plot](./imgs/workflow_with_pics.png)
...
1. Convert all python run scripts in `<OpenSPGen-installation-path>/Python` to executables.
   ```
   cd <OpenSPGen-installation-path>/Python
   chmod +x *py
   ```
...
There are 2 main run scripts that perform SP generation:

1. `RunRepeats.py`: performs sequential conformer jobs and the number of conformers/jobs is specified by the flag `--njobs`.


   ```
   # From the directory you wish your jobs to be saved at
   conda activate spg-env
   RunRepeats.py --idtype SMILES --id C --charge 0 --nslots 4 --name Methane
   ```

   More examples using different molecular identifiers are provided in `run-sp-job.sh` and `run-tests.sh`.

   The below help message summarizes the required and optional user arguments. And it can be generated by running `RunRepeats.py --help`.

   ```
   usage: RunRepeats.py [-h] --idtype IDTYPE --id ID [--charge CHARGE] [--initialxyz INITIALXYZ] [--preoptimize PREOPTIMIZE] [--name NAME]
                     [--nslots NSLOTS] [--njobs NJOBS] [--noautoz NOAUTOZ] [--iodine IODINE]

   options:
   -h, --help            show this help message and exit
   --idtype IDTYPE       Molecule identifier type. Options: SMILES, CAS-Number, InChI, InChIKey, mol2, or xyz (Not case sensitive, but must
                        include separators like `-`). This argument is required.
   ...
   --initialxyz INITIALXYZ
                        Path to an initial xyz file for NWChem geometry optimization. If omitted, a random-seeded geometry is used.
   --preoptimize PREOPTIMIZE
                        Pre-optimize the molecule using a standard forcefield (MMFF94). Options: True or False. Only available if a `mol2`
                        idtype is provided.
...
1. `RunParallel.py`: takes the conformer id as an argument instead of the number of jobs/conformers using the flag `--confid`. This allows submitting conformer jobs separately. This script also requires providing a name for the job to prevent conformer jobs from the same molecule to be saved in different parent directories.

   ```
   # From the directory you wish your jobs to be saved at
   conda activate spg-env
   RunParallel.py --idtype smiles --id CCCCCC --name C6 # uses the default confid=0 and corresponds to a conformer initialized with the seed 42
   RunParallel.py --idtype smiles --id CCCCCC --confid 0 --name C6 # corresponds to a conformer initialized with the seed 42
   RunParallel.py --idtype smiles --id CCCCCC --confid 1 --name C6 # corresponds to a conformer initialized with the seed 43
   RunParallel.py --idtype smiles --id CCCCCC --confid 2 --name C6 # corresponds to a conformer initialized with the seed 44
   ```

   The below help message summarizes the required and optional user arguments. And it can be generated by running `RunParallel.py --help`.

   ```
   usage: RunParallel.py [-h] --idtype IDTYPE --id ID [--charge CHARGE] [--initialxyz INITIALXYZ] [--preoptimize PREOPTIMIZE] --confid CONFID
                        --name NAME [--nslots NSLOTS] [--noautoz NOAUTOZ] [--iodine IODINE]
   ...
   --preoptimize PREOPTIMIZE
                           Pre-optimize the molecule using a standard forcefield (MMFF94). Options: True or False. Only available if a `mol2`
                           idtype is provided.
   --confid CONFID       Conformer ID or job number. Maps the provided index to a random seed for structure intialization. Indexing starts at 0
                           corresponding to a random seed of 42.
   --name NAME           Tail for the job name. Job names are required so that different conformer jobs of the same molecule are saved under the
                           same molecule folder.
   --nslots NSLOTS       Number of cores/threads to use for NWChem calculations. Default is 4.
   --noautoz NOAUTOZ     NWChem setting to disable use of internal coordinates. Default is False.
   --iodine IODINE       The molecule contains an iodine atom. Default is False.
   ```
